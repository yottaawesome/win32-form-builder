...
namespace Designer
{
	...
	// --- Extracted CanvasProc message handlers ---

	auto OnCanvasLButtonDown(DesignState& state, Win32::HWND hwnd, Win32::LPARAM lParam) -> Win32::LRESULT
	{
		Win32::SetFocus(hwnd);
	...
	auto OnCanvasMouseMove(DesignState& state, Win32::HWND hwnd, Win32::LPARAM lParam) -> Win32::LRESULT
	{
		int rawX = Win32::GetXParam(lParam);
		int rawY = Win32::GetYParam(lParam);
		int offset = RulerOffset(state);
		...
		// No drag active — update cursor shape based on hit testing.
		if (state.placementMode)
		{
	...
	auto OnCanvasRButtonUp(DesignState& state, Win32::HWND hwnd, Win32::LPARAM lParam) -> Win32::LRESULT
	{
		int offset = RulerOffset(state);
		int x = Win32::GetXParam(lParam) - offset;
	...
	auto OnCanvasKeyDown(DesignState& state, Win32::HWND hwnd, Win32::WPARAM wParam) -> Win32::LRESULT
	{
		...
		Win32::WPARAM wParam, Win32::LPARAM lParam) -> Win32::LRESULT
	{
		...
		case Win32::Messages::Paint:
		{
			...
			if (state)
			{
				...
				DrawDisabledOverlays(*state, hdc);
		...
		case Win32::Messages::LButtonDown:
			if (state) return OnCanvasLButtonDown(*state, hwnd, lParam);
			break;

		case Win32::Messages::MouseMove:
			if (state) return OnCanvasMouseMove(*state, hwnd, lParam);
			break;
		...
		case Win32::Messages::RButtonUp:
			if (state) return OnCanvasRButtonUp(*state, hwnd, lParam);
			break;

		case Win32::Messages::KeyDown:
			if (state) return OnCanvasKeyDown(*state, hwnd, wParam);
			break;
		}
